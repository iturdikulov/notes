...
Twelve-Factor application is methodology to build web apps (SaaS), required for (main concepts):

- Declarative automation configurations, to minimize time and cost for new developers joining the project.
- Provide portability between execution environments.
...
Where to store app environment specific files?
<span class="f"></span>
...
Is there difference for 12 factor app between local and third party services (attachable resources)?
<span class="f"></span>
No, local and third party services are the "same", you can change local backing service with remote anytime by changing configuration. <!--SR:!2025-08-14,13,248-->
...
Twelve-factor app processes should never demonize or write PID files. Instead, rely on the operating system's process manager (distributed process manager on a cloud platform) to manage output streams, respond to crashed processes and handle user-initiated restarts and shutdowns.
...
Twelve-factor app is designed for continuous deployment by keeping the gap between development and production environment small.
...
In staging or production deploys, each process’ stream will be captured by the execution environment, collated together with all other streams from the app. Then routed to one or more final destinations for viewing and long-term archival (log routers, log indexing and analysis system). App even can't access to final logs destinations. <!--SR:!2024-12-08,2,248-->
...
Admin code must ship with application code to avoid synchronization issues. The same dependency isolation techniques should be used on all process types. How you run usual processes (through some wrapper, or tool), you also run admin processes. 

Python program using Virtualenv should use the vendored bin/python for running both the `webserver` and any `manage.py` admin processes. <!--SR:!2024-12-07,1,228-->

Is REPL shell required for twelve-factor app?
<span class="f"></span>
Twelve-factor strongly favors languages which provide an REPL shell out of the box, and which make it easy to run one-off scripts. 

In a local deploy, developers invoke one-off admin processes by a direct shell command inside the app’s checkout directory. In a production deploy, developers can use ssh or other remote command execution mechanism provided by that deploys execution environment to run such a process. <!--SR:!2025-07-29,5,248-->
